...
function buildPlaneWithBuildingHoles(txLon, txLat) {
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    const circleLon = txLon + (east / metersPerDegLon);
    const circleLat = txLat + (north / metersPerDegLat);
    circlePoints.push(circleLon, circleLat);
  }
  const outerBoundary = Cesium.Cartesian3.fromDegreesArray(circlePoints);

  // Find buildings that intersect with the circle (within MAX_RANGE_M)
  // If buildings array is not available yet, return full circle
  ...
  for (let i = 0; i < buildings.length; i++) {
    ...
    // Check if building intersects the circle using multiple strategies:
    // 1. Centroid within range
    // 2. Any vertex within range
    // 3. Any edge midpoint within range
    // 4. Building polygon contains TX point (building covers TX)
    // 5. Check if any building edge segment intersects circle boundary
    let intersects = false;
    
    // Check if TX is inside building (building covers TX)
    if (_pointInPoly(txLon, txLat, b.verts)) {
      intersects = true;
    } else {
      // Check centroid distance
      const dist = haversineMeters(txLat, txLon, bLat, bLon);
      if (dist <= MAX_RANGE_M * 1.5) { // Use 1.5x margin for large buildings
        // Check if any building vertex is within the circle
        for (const v of b.verts) {
          const vDist = haversineMeters(txLat, txLon, v[1], v[0]);
          if (vDist <= MAX_RANGE_M) {
            intersects = true;
            break;
          }
        }
        
        // Check if any edge midpoint is within range
        if (!intersects) {
          for (let j = 0; j < b.verts.length; j++) {
            const v1 = b.verts[j];
            const v2 = b.verts[(j + 1) % b.verts.length];
            const midLon = (v1[0] + v2[0]) / 2.0;
            const midLat = (v1[1] + v2[1]) / 2.0;
            const midDist = haversineMeters(txLat, txLon, midLat, midLon);
            if (midDist <= MAX_RANGE_M) {
              intersects = true;
              break;
            }
          }
        ...
        // Check if building centroid is within range (for small buildings)
        if (!intersects && dist <= MAX_RANGE_M) {
          intersects = true;
        }
      }
    }
    ...
    if (intersects) {
      // Add building footprint as a hole
      // For Cesium holes, we need to reverse the winding order (clockwise for holes)
      const holePoints = [];
      // Reverse the vertices to create a hole (clockwise winding for holes in Cesium)
      for (let j = b.verts.length - 1; j >= 0; j--) {
        holePoints.push(b.verts[j][0], b.verts[j][1]);
      }
      const holeBoundary = Cesium.Cartesian3.fromDegreesArray(holePoints);
      buildingHoles.push(holeBoundary);
    }
  }

  // Create polygon hierarchy with outer boundary and holes
  // Cesium polygon hierarchy format: { positions: outerBoundary, holes: [hole1, hole2, ...] }
  if (buildingHoles.length > 0) {
    return {
      positions: outerBoundary,
      holes: buildingHoles
    };
  } else {
    // No holes, just return the outer boundary as a simple array
    return outerBoundary;
  }
}
  ...
  if (!_txSphere) {
    _txSphere = viewer.entities.add({
      ...
      polygon: {
        ...
        outline: true,
        outlineColor: Cesium.Color.YELLOW.withAlpha(0.40),
        // Enable depth testing so buildings can visually block the plane

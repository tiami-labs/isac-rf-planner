...
async function setTxMarker(lon, lat) {
  ...
  // TX emission sphere (3D analog of the 2D coverage circle)
  // This is purely visual; the RF recompute uses _txLon/_txLat and MAX_RANGE_M.
  if (!_txSphere) {
    _txSphere = viewer.entities.add({
      name: 'tx_sphere',
      position: pos,
      ellipsoid: {
        radii: new Cesium.Cartesian3(MAX_RANGE_M, MAX_RANGE_M, MAX_RANGE_M),
        material: Cesium.Color.YELLOW.withAlpha(0.05),
        outline: true,
        outlineColor: Cesium.Color.YELLOW.withAlpha(0.20)
      }
    });
  } else {
    _txSphere.position = pos;
  }
}

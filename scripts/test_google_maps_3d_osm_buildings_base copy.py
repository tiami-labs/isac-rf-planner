...
function buildPlaneWithBuildingHoles(txLon, txLat) {
  // Radial wave/explosion with line-of-sight blocking
  // Cast rays from TX in all directions, stop at first building intersection
  const numRays = 128; // More rays = smoother deformation  
  ...
  // If no buildings, return full circle
  if (!buildings || buildings.length === 0) {
    ...
    return new Cesium.PolygonHierarchy(Cesium.Cartesian3.fromDegreesArray(wavePoints));
  }
  
  // For each ray, find first building intersection
  // DEBUG: Log building count
  console.log(`[TX Plane] Checking ${buildings ? buildings.length : 0} OSM buildings for ray intersections`);
  ...
  for (let i = 0; i < numRays; i++) {
    ...
    let rayDist = MAX_RANGE_M;
    let blockedByBuilding = false;
    
    // Check all buildings to find closest intersection along this ray
    for (let j = 0; j < buildings.length; j++) {
      const b = buildings[j];
      if (!b.verts || b.verts.length < 3) continue;
      
      // Binary search for intersection point along ray
      let minDist = 0;
      let maxDist = MAX_RANGE_M;
      let closestIntersection = MAX_RANGE_M;
      
      // Sample points along ray to find intersection
      for (let step = 0; step < 50; step++) {
        const testDist = minDist + (maxDist - minDist) * (step / 50.0);
        const testLon = txLon + (dirX * testDist / metersPerDegLon);
        const testLat = txLat + (dirY * testDist / metersPerDegLat);
        
        // Check if this point is inside building polygon (2D footprint check)
        if (_pointInPoly(testLon, testLat, b.verts)) {
          closestIntersection = Math.min(closestIntersection, testDist);
          maxDist = testDist; // Intersection found, search closer to TX
          // Don't break - continue to find the closest intersection point
        }
      }
      
      // Also check if ray segment intersects building edges
      const endLon = txLon + (dirX * closestIntersection / metersPerDegLon);
      const endLat = txLat + (dirY * closestIntersection / metersPerDegLat);
      
      // Check segment intersection with building polygon
      if (segmentIntersectsPolygon(txLon, txLat, endLon, endLat, b.verts)) {
        // Find exact intersection point by binary search
        let low = 0;
        let high = closestIntersection;
        let bestDist = closestIntersection;
        
        for (let iter = 0; iter < 20; iter++) {
          const mid = (low + high) / 2.0;
          const midLon = txLon + (dirX * mid / metersPerDegLon);
          const midLat = txLat + (dirY * mid / metersPerDegLat);
          
          if (segmentIntersectsPolygon(txLon, txLat, midLon, midLat, b.verts)) {
            bestDist = mid;
            high = mid;
          } else {
            low = mid;
        ...
        closestIntersection = Math.min(closestIntersection, bestDist);
      }
      
      // Update ray distance to closest intersection
      if (closestIntersection < MAX_RANGE_M) {
        blockedByBuilding = true;
      }
      rayDist = Math.min(rayDist, closestIntersection);
    }
    
    // DEBUG: Log if ray was blocked
    if (blockedByBuilding && rayDist < MAX_RANGE_M * 0.9) {
      // Ray was significantly blocked
    }
    
    // Add point at the stopped distance
    const finalLon = txLon + (dirX * rayDist / metersPerDegLon);
    const finalLat = txLat + (dirY * rayDist / metersPerDegLat);
    wavePoints.push(finalLon, finalLat);
  }
  
  // DEBUG: Count how many rays were blocked
  const blockedRays = wavePoints.filter((p, i) => {
    if (i % 2 === 0) {
      const lon = wavePoints[i];
      const lat = wavePoints[i + 1];
      const dist = haversineMeters(txLat, txLon, lat, lon);
      return dist < MAX_RANGE_M * 0.95;
    }
    return false;
  }).length / 2;
  console.log(`[TX Plane] ${blockedRays} rays were blocked by OSM buildings`);
...
async function setTxMarker(lon, lat) {
  ...
  // Build the plane with building holes (deformations)
  const hierarchy = buildPlaneWithBuildingHoles(lon, lat);

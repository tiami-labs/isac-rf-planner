...
function buildPlaneWithBuildingHoles(txLon, txLat) {
  ...
  for (let i = 0; i < numRays; i++) {
    ...
    for (let j = 0; j < buildings.length; j++) {
      ...
      // Check if ray from TX to MAX_RANGE intersects this building
      const maxLon = txLon + (dirX * MAX_RANGE_M / metersPerDegLon);
      const maxLat = txLat + (dirY * MAX_RANGE_M / metersPerDegLat);
      
      // First check: does the ray segment intersect the building polygon?
      if (segmentIntersectsPolygon(txLon, txLat, maxLon, maxLat, b.verts)) {
        // Binary search to find exact intersection point
        let low = 0;
        let high = MAX_RANGE_M;
        let bestDist = MAX_RANGE_M;
        ...
        for (let iter = 0; iter < 30; iter++) {
          ...
          if (segmentIntersectsPolygon(txLon, txLat, midLon, midLat, b.verts)) {
            bestDist = mid;
            high = mid; // Intersection found, search closer to TX
          } else {
            low = mid; // No intersection yet, search further from TX
          }
        }
        
        // Update ray distance to closest intersection
        rayDist = Math.min(rayDist, bestDist);
      } else {
        // Ray doesn't intersect building edges, but check if TX is inside building
        if (_pointInPoly(txLon, txLat, b.verts)) {
          // TX is inside building, ray stops immediately
          rayDist = 0;
        }
      }
    }
